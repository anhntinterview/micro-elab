...
class Client {
  ...
  constructor(config: Config) {
    ...
    console.log(`config in constructor: `,this.config);
  }
  ...
  shouldRetryCondition(
    ...
    //      At the moment, 500 will be returned on all errors, that are not of type AppError.
  ...
  normalizeHeaders(obj: Record<string, any>): Record<string, any> {
    ...
    return Object.keys(obj).reduce(
      (result: Record<string, any>, header: string) => {
  ...
  async request(
    ...
    const reqOpts: {
      ...
      data?: Record<string, any>; // optional data type
    } = {
      method,
